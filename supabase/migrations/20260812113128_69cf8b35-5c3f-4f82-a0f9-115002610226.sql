...
BEGIN
  ...
  VALUES (
    trim(_customer_name), trim(_phone), coalesce(_address, ''),
    nullif(trim(coalesce(_delivery_date, '')), '')::date,
    coalesce(_delivery_slot, ''), coalesce(_comment, ''), v_total, coalesce(_kind, 'order')
  )
